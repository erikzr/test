...
<body>
 ...
 <main class="main-content">
    ...
    <?php
    // Menentukan halaman mulai dan berakhir berdasarkan posisi halaman saat ini
    $start = max(1, $currentPage - 5); // Halaman mulai 5 halaman sebelumnya
    $end = min($totalPages, $start + 9); // Halaman berakhir 5 halaman setelah halaman mulai (untuk total 10)

    // Jika jumlah total halaman kurang dari 10, sesuaikan range untuk menampilkan semua halaman yang ada
    if ($totalPages < 10) {
        $start = 1;
        $end = $totalPages;
    }

    // Menampilkan pagination 10 halaman atau total halaman yang ada
    for ($i = $start; $i <= $end; $i++): ?>
